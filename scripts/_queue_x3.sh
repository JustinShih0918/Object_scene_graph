...
for scene in 00848-ziup5kvtCCR 00829-QaLdnwvtxbs 00880-Nfvxx8J5NCo; do
  echo "[$(date -Is)] START X3 $scene" >> outputs/room_campaign/campaign.log
  python scripts/run_eval.py +experiment=ycb_dynamic_room_floor \
    ...
    eval.debug_frames=false eval.save_viz=false +run_tag=X3 \
    > outputs/room_campaign/X3_$scene.log 2>&1
  echo "[$(date -Is)] END X3 $scene exit=$?" >> outputs/room_campaign/campaign.log
done
echo "[$(date -Is)] X3 DONE" >> outputs/room_campaign/campaign.log
