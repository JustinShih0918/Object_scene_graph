...
for scene in 00848-ziup5kvtCCR 00880-Nfvxx8J5NCo 00829-QaLdnwvtxbs; do
  echo "[$(date -Is)] START P $scene" >> outputs/room_campaign/campaign.log
  python scripts/run_eval.py +experiment=ycb_dynamic_prox \
    ...
    eval.debug_frames=false eval.save_viz=false +run_tag=P \
    > outputs/room_campaign/P_$scene.log 2>&1
  echo "[$(date -Is)] END P $scene exit=$?" >> outputs/room_campaign/campaign.log
done
echo "[$(date -Is)] P DONE" >> outputs/room_campaign/campaign.log
