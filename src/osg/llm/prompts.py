...
ROOM_PRIOR_SYSTEM = (
    "You reason about where a household object has been moved to. You are given "
    "a robot's map of a house, room by room, with what it has already looked at "
    "and what it now believes has gone missing. Answer with JSON only."
)

ROOM_PRIOR_USER = """A {target} is missing. {anchor_text}
Rooms in the house, each listed by the furniture mapped in it:
{rooms_text}

Rooms are numbered, not named: work out what kind of room each one is from the
furniture it holds, then order EVERY room id above by how likely someone put the
{target} there. Most likely first.
Respond as JSON: {{"rooms": [<room id>, <room id>, ...]}}"""


# The same question, asked about a PERSON rather than about the object.
#
# `ROOM_PRIOR_USER` asks where a {target} belongs, and the model answers it well:
# for six YCB targets it put the room holding the refrigerator first, six times
# out of six. That is the right answer to the wrong question. The evaluated
# layouts (`outputs/substituted_layouts`, imported from the collector, NOT
# generated by author_semantic_layouts.py) put relocated objects where a person
# set them down -- 00848's cross_anchor destinations are bed 12 and nightstand 3,
# 00829's are bed 5 / sink 2 / desk 2, 00880's are desk 4 / sink 2 / bed 2. The
# tomato soup can is on a bed in all three of 00848's cross_anchor layouts, and
# "where does a tin can belong" ranks first the one room with no bed in it.
#
# So this asks what actually happened instead: someone was carrying it and put it
# down on whatever surface was to hand. Measured on the same five rooms, the two
# framings invert each other -- "belongs" ranks the bed-less room FIRST 6/6,
# "placement" ranks it LAST.
ROOM_PRIOR_PLACEMENT_SYSTEM = (
    "You reason about where a person left a household object while moving around "
    "their home. Answer with JSON only."
)

ROOM_PRIOR_PLACEMENT_USER = """Someone carried a {target} around this house and set it down somewhere, then left it there. {anchor_text}
Rooms in the house, each listed by the furniture mapped in it:
{rooms_text}

People put things down on whatever flat surface is to hand where they happen to be
standing -- a bed, a nightstand, a desk, a chest of drawers -- not where the object
belongs. Rooms are numbered, not named. Order EVERY room id above by how likely it
is that the {target} was set down there and left.
Respond as JSON: {{"rooms": [<room id>, <room id>, ...]}}"""
